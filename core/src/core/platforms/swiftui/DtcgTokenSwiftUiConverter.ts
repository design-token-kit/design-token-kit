...
export class DtcgTokenSwiftUiConverter implements TokenSwiftUiConverter {
    ...
    #renderBaseEnum(doc: Dtcg): string {
        ...
        const desc = renderDocDescription(doc.root.description, 0);
    ...
    #renderStructLayer(list: DtcgList): string {
        ...
        const instances = list.themes.size > 0
            ...
            : [this.#renderInstance("light", ROOT, list.base.root)];
        const extension = ["extension Theme {", ...instances.map((line) => indentBlock(line, 1)), "}"].join("\n");
        return [structDef, "", extension].join("\n");
    }
